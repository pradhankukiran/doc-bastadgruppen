...
const DocPdfTemplate: React.FC<Props> = ({ formData, languages }) => {
  const { productInfo, selectedBodyDetails, complianceInfo } = formData;

  // Languages that need NotoSans for extended Latin characters
  const extendedLangs = [
    "lv",
    "cs",
    "sk",
  ...
  const getFontFamily = (language: string) =>
    extendedLangs.includes(language) ? "NotoSans" : "Lato";
